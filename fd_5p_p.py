...
__all__ = ["fd_5p_p_create_matrix", "fd_5p_p_solver", "fd_5p_p_solver_extrap"]

#***************************
#library functions

def fd_5p_p_create_matrix(potential, xmin, xmax, N):
  """Create the matrix for the finite difference 5 points discretization
  ...
  if N<3:
    print("ERROR: N need to be at least 3!")
    sys.exit(1)
  ...
  for i in range(0, N, 1):
    M[i][i]=5.0/2.0+step*step*potential(xmin+i*step)
...
def fd_5p_p_solver(potential, xmin, xmax, N):
  """Find the eigenvalues of the Schrodinger equation
  -\psi''(x)+V(x)\psi(x)=E\psi(x)
  using a finite difference 5 point discretization with
  ...
  M=fd_5p_p_create_matrix(potential, xmin, xmax, N)
...
def fd_5p_p_solver_extrap(potential, xmin, xmax, N):
  """Find the eigenvalues of the Schrodinger equation
  -\psi''(x)+V(x)\psi(x)=E\psi(x)
  using a finite difference 5 point discretization with
  ...
  ris1=fd_5p_p_solver(potential, xmin, xmax, N)
  ris2=fd_5p_p_solver(potential, xmin, xmax, 2*N)

  aux=ris2[:N]
  
  ris=(16.0*aux-ris1)/15.0
...
if __name__=="__main__":
  ...
  risA=fd_5p_p_solver(pot_h, xmin, xmax, N)
  risB=fd_5p_p_solver(pot_h, xmin, xmax, 2*N)
  risC=fd_5p_p_solver(pot_h, xmin, xmax, 3*N)
  risD=fd_5p_p_solver(pot_h, xmin, xmax, 4*N)

  risB_extrap=fd_5p_p_solver_extrap(pot_h, xmin, xmax, 2*N)

  print('Harmonic oscillator on [{:+.2f}, {:+.2f}]'.format(xmin, xmax))
  print('5 points finite difference with periodic boundary conditions, N={:d}'.format(N))
  print('')
  ...
  risA=fd_5p_p_solver(pot_zero, xmin, xmax, N)
  risB=fd_5p_p_solver(pot_zero, xmin, xmax, 2*N)
  risC=fd_5p_p_solver(pot_zero, xmin, xmax, 3*N)
  risD=fd_5p_p_solver(pot_zero, xmin, xmax, 4*N)

  risB_extrap=fd_5p_p_solver_extrap(pot_zero, xmin, xmax, 2*N)
