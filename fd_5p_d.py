...
__all__ = ["fd_5p_d_create_matrix", "fd_5p_d_solver", "fd_5p_d_solver_extrap"]

#***************************
#library functions

def fd_5p_d_create_matrix(potential, xmin, xmax, N):
...
def fd_5p_d_solver(potential, xmin, xmax, N):
  ...
  M=fd_5p_d_create_matrix(potential, xmin, xmax, N)
...
def fd_5p_d_solver_extrap(potential, xmin, xmax, N):
  """Find the eigenvalues of the Schrodinger equation
  -\psi''(x)+V(x)\psi(x)=E\psi(x)
  using a finite difference 5 point discretization with
  ...
  ris1=fd_5p_d_solver(potential, xmin, xmax, N)
  ris2=fd_5p_d_solver(potential, xmin, xmax, 2*N)
...
if __name__=="__main__":
  ...
  risA=fd_5p_d_solver(pot_h, xmin, xmax, N)
  risB=fd_5p_d_solver(pot_h, xmin, xmax, 2*N)
  risC=fd_5p_d_solver(pot_h, xmin, xmax, 3*N)
  risD=fd_5p_d_solver(pot_h, xmin, xmax, 4*N)

  risB_extrap=fd_5p_d_solver_extrap(pot_h, xmin, xmax, 2*N)

  print('Harmonic oscillator on [{:+.2f}, {:+.2f}]'.format(xmin, xmax))
  print('5 points finite difference with Dirichlet boundary conditions, N={:d}'.format(N))
  print('')
